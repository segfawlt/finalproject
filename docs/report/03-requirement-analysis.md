...
The system has one primary actor and three supporting actors. The use cases are
divided into three diagrams so that each workflow remains readable.

| Actor             | Type                        | Role in the system                                                                                                                  |
| ----------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Administrator** | Primary (human)             | A signed-in Discord user holding _Manage Server_ on the target guild. Initiates every user-facing workflow.                         |
| **Discord API**   | Supporting (external)       | Provides OAuth login, guild and permission data, and applies live changes through the bot. It is also the source of external drift. |
| **LLM Provider**  | Supporting (external)       | Produces and revises plans, checks plans against server rules at execution, and assists with template merge and stale-plan repair.  |
| **Scheduler**     | Supporting (time-triggered) | Periodically initiates drift detection. It represents the passage of time rather than a human user.                                 |
...
a fresh conflict check, Stage 1 deterministic validation, and the Stage 2 LLM
policy check against the stored server rules. The ordinary planning prompt does
not currently receive those rules. This diagram shows the additional actors and
flows introduced by execution: all execution and rollback operations contact
Discord. The **Check pre-execution conflicts** and **Validate plan** use cases
are included automatically in UC-10, ensuring deterministic revalidation happens
before any live changes. **Attempt structural rollback** can be triggered by
explicit user request (UC-11) or by execution failure (UC-17).
...
| **Primary actor**    | Administrator                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Secondary actors** | Discord API, LLM Provider (Stage 2 policy validation)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Exception flows**  | _E1 — Stale view (FR-21):_ server changed since planning → refuse with a 409. _E2 — Conflict check fails:_ a current-state assumption fails → refuse with the list of conflicts; plan stays _draft_. _E3 — Validation fails (FR-15, FR-16):_ blockers found → refuse with blockers and warnings; plan stays _draft_. _E4 — Server busy:_ lock held by another plan → refuse. _E5 — Step failure (FR-20):_ a step fails after retries → System attempts convergence to the before-snapshot and records the execution and rollback outcome. _E6 — Overall timeout (NFR-9):_ execution exceeds the overall deadline → stop waiting, attempt rollback, and release the lock.                                                                                                                          |
...
applied. Discord.js cannot cancel an operation already sent to Discord, so a
...
| **Primary actor**    | Administrator                                                                                                                                                                                                                                                                                                                                              |
| **Secondary actors** | Scheduler, Discord API, LLM Provider (only when AI repair is selected)                                                                                                                                                                                                                                                                                     |
...
| **Main flow**        | 1. System lists the server's rules. 2. Administrator enters or changes rule text. 3. System validates and saves the change. 4. System displays the updated rule set. 5. At a later execution, Stage 2 loads the stored rules for the policy check; ordinary planning does not automatically receive them. |
...
| **Realises**         | FR-26                                                                                                                                                                                                                                                                             |
| **Primary actor**    | Administrator                                                                                                                                                                                                                                                                     |
| **Secondary actors** | LLM Provider (only when merge is selected)                                                                                                                                                                                                                                        |
| **Precondition**     | Administrator is signed in and may manage the selected server.                                                                                                                                                                                                                    |
...
| **Realises**         | FR-14                                                                                                                                                                                                                                                                                                                                       |
| **Primary actor**    | Administrator                                                                                                                                                                                                                                                                                                                               |
| **Secondary actors** | LLM Provider (only for merge)                                                                                                                                                                                                                                                                                                               |
...
of Conduct, the relevant professional standard for this work.
...
The relevant principles include _lawfulness, fairness and transparency_, _data
...
legible before approval — but they mitigate rather than remove the risk that a
...
finish after the engine stops waiting because Discord.js provides no
...
Conduct: the public interest, professional competence and integrity, duty to
...
retention or erasure policy, OAuth tokens stored in plaintext — would need to be
...
pragmatically chosen (not empirically derived) timeout values, the reliance on
reviewer attention — instead of overstating the guarantees.

...
not currently receive those rules. When rules exist, Stage 2 fails closed if
...
| **Exception flows**  | _E1 — Stale view (FR-21):_ server changed since planning → refuse with a 409. _E2 — Conflict check fails:_ a current-state assumption fails → refuse with the list of conflicts; plan stays _draft_. _E3 — Validation fails (FR-15, FR-16):_ blockers found, or configured server rules cannot be loaded or evaluated → refuse with blockers and warnings; plan stays _draft_. A guild with no configured rules skips Stage 2. _E4 — Server busy:_ lock held by another plan → refuse. _E5 — Step failure (FR-20):_ a step fails after retries → System attempts convergence to the before-snapshot and records the execution and rollback outcome. _E6 — Overall timeout (NFR-9):_ execution exceeds the overall deadline → stop waiting, attempt rollback, and release the lock.                |

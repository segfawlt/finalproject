...
test.describe("ST-08: Stale plan rejection", () => {
  test("executing on mismatched guild returns 409 conflict on stale state", async ({
    request,
  }) => {
  ...
  test.fixme(
    "executing after external Discord edit returns 409 with canAIRepair",
    async ({ request }) => {
      // Requires: mutate Discord externally between conversation creation and execute.
      // Cannot be automated without bot-level Discord mutation capability.
    }
  );
});
...
test.describe("ST-09: Cancel planning while LLM is active", () => {
  test("cancelling an active conversation sets status to cancelled", async ({ request }) => {
    ...
    const cancelRes = await request.post(
      `/api/guilds/${GUILD_ID}/conversations/${convId}/cancel`
    );
